...
interface Props {
  entry: Entry;
  /** For inline-edit of the management summary. */
  editTable?: string;
}
...
export function RegulatorikDetailPanel({ entry, editTable }: Props) {
  const [tab, setTab] = React.useState<'was' | 'todo' | 'risk'>('was');
  const aufwand = (entry.aufwand_tshirt ?? '').trim().toUpperCase();

  return (
    <div className="space-y-5">
      {/* Management Summary (inline-editierbar) */}
      {entry.beschreibung_einsteiger && (
        <ManagementSummaryCallout
          initialValue={entry.beschreibung_einsteiger}
          editTable={editTable}
          itemId={entry.id ?? undefined}
          fieldKey="beschreibung_einsteiger"
        >
