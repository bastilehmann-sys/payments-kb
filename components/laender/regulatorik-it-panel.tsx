...
function RegulationCard({ row }: { row: CountryBlockRow }) {
  ...
  return (
    <article className={cn('rounded-xl border bg-card border-l-4 shadow-sm', style.accent)}>
      ...
      {summary && (
        <p className="mt-3 px-5 text-base leading-relaxed text-foreground/85">{linkify(summary)}</p>
      )}
      ...
      {open && (
        <div className="space-y-4 border-t border-border/60 bg-muted/20 px-5 py-4 text-base">
